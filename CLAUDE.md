# construction-schedule — Claude への指示
...
- `Obsidian Vault/Projects/construction-schedule/README.md` — 本プロジェクト
- `Obsidian Vault/Projects/construction-schedule/Mistakes/` — 既知の地雷
...
| 種類 | パス |
|------|------|
| ミスログ | `Obsidian Vault/Projects/construction-schedule/Mistakes/YYYY-MM-DD_<topic>.md` |
| ADR | `Obsidian Vault/Projects/construction-schedule/Decisions/YYYY-MM-DD_<topic>.md` |
| ノウハウ | `Obsidian Vault/Projects/construction-schedule/Knowledge/<topic>.md` |
| セッション | `Obsidian Vault/Projects/construction-schedule/Sessions/YYYY-MM-DD_<topic>.md` |
...
1. 🔴 **「ダメ」を 2 回連続で受けたらコード読むまでデプロイ禁止** — ログ追加で原因特定は 1 周まで
2. 🟡 **UI 修正指示は「対象 / 属性 / 期待値」を 1 行復唱してから着手**
3. 🟡 **バッジは `inline-flex + height:28px + icon 12px` をデフォ適用** — `align-items:center; gap:6px; line-height:1;`
4. 🟡 **「Ctrl+Shift+R / unregister」を完了報告に書かない** — サーバー側で実証してから

## 5. Magic Phrases（強制トリガー）

| フレーズ | 動作 |
|---------|------|
| 「失敗として記録」「ミスに残」 | `Projects/construction-schedule/Mistakes/` に作成 + push |
| 「ADR にして」「決定を残」 | `Projects/construction-schedule/Decisions/` に作成 + push |
